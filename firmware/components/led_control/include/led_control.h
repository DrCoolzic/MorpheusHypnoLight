 ...
 *
 * The output duty is calculated as osc_value × clamp(global_brightness ×
 * gamma(current_brightness) + low_end_lift(current_brightness)), where gamma
 * applies a perceptual correction and low_end_lift adds a small, saturating
 * offset so the LED turn-on threshold stays around the same brightness value
 * for any global dimmer setting. Finite values outside the normalized range are
